...
請以 JSON 格式回應，提供完整的 7 項分析：
{
  ...
  try {
    ...
    return {
      ...
      coreConclusion: `5期策略：優先推薦 ${hot3.join(", ")} 等熱號，搭配連莊號 ${streakDisplay}，避開死碼 ${deadDisplay}，透過精準排除死碼並輔以區間分布平衡，以期在短期內達到最佳選號效果。`,
      strategy: `綜合熱號共振、連莊趨勢、斜連交會點及區間平衡，${hot3.join(", ")}為當前最具潛力之黃金球。${hot3[0]}為強勢熱號且尾數共振，${hot3[1]}為關鍵斜連交會點，${hot3[2]}為持續連莊號。策略邏輯為：以近期高頻率開出的號碼為核心，結合尾數共振與斜連趨勢，鎖定高機率落球，同時排除死碼以提高精準度。`,
      sampleCount: draws.length,
      usedLLM: false,
      parseErrors,
    };
